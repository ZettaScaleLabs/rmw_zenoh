...
{
  SerializationBufferPool * buffer = reinterpret_cast<SerializationBufferPool *>(context);
  buffer->deallocate(static_cast<uint8_t *>(data));
}
  ...
  std::optional<z_owned_shm_provider_t> & shm_provider)
{
  ...
  if (shm_provider.has_value()) {
    ...
    msg_bytes = this->serialization_buffer_pool.allocate(allocator, max_data_length);
  ...
  if (shmbuf.has_value()) {
    ...
    z_bytes_from_buf(
      &payload, msg_bytes, data_length, delete_z_bytes,
      reinterpret_cast<void *>(&this->serialization_buffer_pool));
  }
